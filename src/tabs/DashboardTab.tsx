...
type SortKey = 'symbol' | 'lastClose' | 'dayChange' | 'predicted' | 'expectedChange' | 'probUp';
...
const COLUMNS: { key: SortKey; label: string; hint: string; numeric: boolean }[] = [
  ...
  { key: 'dayChange', label: 'Today', hint: 'Change on the latest trading day', numeric: true },
  ...
  { key: 'probUp', label: 'Chance up', hint: "Share of simulations ending above today's close", numeric: true },
];
...
export function DashboardTab({ manifest, fetched, watchlist, onOpen }: Props) {
  ...
  const rows = useMemo(() => {
    const all: Row[] = [
      ...
      ...browserOnly.map((stock, i): Row => {
        const s = summaries[i];
        if (s?.status === 'ready') return { ...stock, ...s.summary, source: 'fetched', note: null };
        return { ...stock, ...empty, source: 'fetched', note: s?.status === 'error' ? s.message : 'Loading…' };
      }),
    ];
  ...
  }, [entries, browserOnly, summaries, query, filter, sort]);
  ...
  function toggleSort(key: SortKey) {
    setSort((s) => (s.key === key ? { key, descending: !s.descending } : { key, descending: key !== 'symbol' }));
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card title="Today's sheet" subtitle="Saved automatically every weekday evening, so the Tracker can compare it with what really happens.">
            ...
            <table className="w-full min-w-[600px] text-sm">
              <thead className="sticky top-0 z-10 bg-ink-900">
                <tr className="text-xs text-ink-400">
                  ...
                  {COLUMNS.map((c) => (
                    <th key={c.key} className={`py-2 pr-3 font-medium whitespace-nowrap ${c.numeric ? 'text-right' : 'text-left'}`} aria-sort={sort.key === c.key ? (sort.descending ? 'descending' : 'ascending') : undefined}>
                  ...
                  <th className="hidden py-2 pr-2 text-right font-medium xl:table-cell">Source</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((r) => {
                    ...
                    <tr key={r.symbol} onClick={() => onOpen(r.symbol)} className="cursor-pointer border-t border-ink-800 hover:bg-ink-800/40">
                      ...
                      <td className="py-2.5 pr-3">
                        <button type="button" className="rounded text-left focus-visible:ring-2 focus-visible:ring-accent-500 focus-visible:outline-none">
                          ...
                          <span className="block max-w-48 truncate text-xs text-ink-400">{r.note ?? r.name}</span>
                        </button>
                      </td>
                      <td className={cell}>{r.lastClose == null ? '—' : formatPrice(r.lastClose, r.symbol)}</td>
                      <td className={`${cell} ${tone(r.dayChange)}`}>{r.dayChange == null ? '—' : formatPct(r.dayChange, true)}</td>
                      ...
                      <td className={cell}>{r.probUp == null ? '—' : formatPct(r.probUp)}</td>
                      <td className="hidden py-2.5 pr-2 text-right xl:table-cell">
