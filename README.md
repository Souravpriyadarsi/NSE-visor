...
- **Algo trading**: a paper-trading bot that runs the Minimal Margin Maximus rules on live 5-minute prices for up to 5 stocks, with a pretend cash account and Zerodha delivery charges. It buys at every close, sells on the trailing stops (always assuming the less favourable order inside a bar), and keeps its orders, daily record and account value in the browser. It replays real prices, so reopening the page catches up on every day it missed (up to 60 days). Stopping it sells everything at the latest price.
