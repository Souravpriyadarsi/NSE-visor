      ...
      for (const period of TEST_PERIODS) {
        ...
        if (result) {
          results[summaryKey(amount, withCosts, period.key)] = [
            ...
            result.overnight.trades,
            result.intraday.trades,
          ];
        }
      }
    }
...
for (const withCosts of [false, true]) {
  const cells = summary.stocks.map((s) => s.results[summaryKey(10000, withCosts, '1Y')]).filter(Boolean);
  const median = (i: number) => cells.map((c) => c[i]).sort((a, b) => a - b)[Math.floor(cells.length / 2)];
  console.log(
    `₹10,000 over 1 year ${withCosts ? 'after' : 'before'} charges, median of ${cells.length} stocks: ` +
